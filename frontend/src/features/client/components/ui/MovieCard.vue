...
<template>
  <div
    class="group relative bg-zinc-900 rounded-2xl overflow-hidden border border-white/5 shadow-2xl transition-all hover:-translate-y-2"
  >
    <router-link
      :to="{
        name: 'movie-detail',
    ...
    >
      <div class="aspect-[2/3] relative overflow-hidden">
        ...
        >
          <button
            class="w-full py-2.5 bg-red-600 text-white rounded-xl font-semibold flex items-center justify-center gap-2 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300"
          >
        ...
        <div class="absolute top-2 left-2 px-2 py-1 bg-red-600 rounded-lg">
          <span class="text-[10px] font-black text-white">{{ movie.rating ?? 'P' }}</span>
        </div>
      </div>
      <div class="p-4">
        ...
        <div class="flex items-center gap-3 mt-1 text-zinc-500 text-xs">
          <span class="flex items-center gap-1">
            <Clock class="w-3 h-3" />
            {{ movie.duration_minutes }} {{ languageStore.t('movie.duration') }}
          </span>
          <span>•</span>
          <span class="truncate">
