...
const canCreate = computed(() => can(ADMIN_PERMISSIONS.ROOMS, ADMIN_ACTIONS.CREATE))
const canUpdate = computed(() => can(ADMIN_PERMISSIONS.ROOMS, ADMIN_ACTIONS.UPDATE))
const canDelete = computed(() => can(ADMIN_PERMISSIONS.ROOMS, ADMIN_ACTIONS.DELETE))
const typeColor = {
  '2D': 'primary',
  '3D': 'info',
  IMAX: 'success',
...
<template>
  <n-space justify="space-between" class="mb-4">
    ...
    <n-button v-if="canCreate" type="primary" @click="openCreateModal">+ Tạo phòng</n-button>
  </n-space>
