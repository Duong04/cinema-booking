...
function createColumns(): DataTableColumns<Role> {
  return [
    { type: 'selection' },
    { title: 'Name', key: 'name' },
    { title: 'Description', key: 'description', render: (row) => h('span', row.description || 'N/A') },
    { title: 'Created At', key: 'created_at', render: (row) => h('span', formatDate(row.created_at)) },
    { title: 'Updated At', key: 'updated_at', render: (row) => h('span', formatDate(row.updated_at)) },
    {
      title: 'Actions',
      key: 'actions',
      render: (row) => h('div', { style: 'display: flex; gap: 8px' }, [
        h(NButton, { size: 'small', type: 'primary', onClick: () => openEditModal(row) }, { default: () => 'Edit' }),
...
<template>
  <n-space justify="space-between" class="mb-4">
    <n-input v-model:value="filters.search" placeholder="Search by name..." clearable style="width: 300px" />
