...
function openEditModal(row: City) {
...
function handleDelete(row: City) {
  dialog.warning({
    title: 'Xác nhận xóa',
    content: `Bạn có chắc chắn muốn xóa thành phố "${row.name}" không?`,
    positiveText: 'Xóa',
    negativeText: 'Hủy',
    onPositiveClick: async () => {
      try {
        ...
        message.error('Đã có lỗi xảy ra khi xóa thành phố')
      }
    },
  })
}
...
function handleDeleteMultiple() {
  ...
  dialog.warning({
    ...
    onPositiveClick: async () => {
      try {
        await Promise.all(checkedRowKeysRef.value.map((id) => deleteCity(id as string)))
        message.success(`Đã xóa ${count} thành phố thành công`)
        checkedRowKeysRef.value = []
        fetchCities()
      } catch {
        message.error('Đã có lỗi xảy ra khi xóa các thành phố')
      }
    },
  })
}
...
<template>
  ...
  <n-data-table
    ...
    :row-key="(row: City) => row.id"
    remote
    @update:checked-row-keys="(keys: DataTableRowKey[]) => (checkedRowKeysRef = keys)"
  />

  <CityFormModal v-model:show="showModal" :city="selectedCity" @success="fetchCities" />
</template>
